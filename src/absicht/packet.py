...
``ab packet``'s core (docs/tasks/31-packet-assembly.md), below the CLI that
renders it: given an already-resolved ``Design`` and its ``Index``, select what
an implementer needs and nothing they do not —
...
def assemble(
    ...
    *elements* the packet carries; the obligations are derived from the
    ...
    elements = tuple(
        PacketElement(ref=ref, fidelity=fidelity, element=_carried(element, fidelity))
...
def _carried(element: Element, fidelity: Fidelity) -> dict[str, object]:
    ...
    seam. ``mode="json"`` keeps the dump serializable as-is."""
